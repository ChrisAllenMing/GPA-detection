...
# define the cityscapes for multi-class detection
class city_multi(imdb):
    def __init__(self, image_set, num_shot=None):
        imdb.__init__(self, 'city_multi_' + image_set)
        self.num_shot = num_shot
        self._image_set = image_set
        self._data_path = os.path.join('data', 'city')
    ...
    def image_path_from_index(self, index):
        ...
        """
        image_path = os.path.join(self._data_path, 'img_' + self._image_set,
    ...
    def _load_image_set_index(self):
        ...
        if self.num_shot is not None:
            np_image_index = np.array(image_index)
            np.random.shuffle(np_image_index)
            image_index = np_image_index[:self.num_shot].tolist()
    ...
    def _load_rpn_roidb(self, gt_roidb):
        ...
        with open(filename, 'rb') as f:
            box_list = pickle.load(f, encoding='iso-8859-1')
    ...
    def _load_city_annotation(self, index):
        ...
        format.
        """
        filename = os.path.join(self._data_path, 'anno_multi_' + self._image_set, index + '.xml')
